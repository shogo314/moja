...
template <typename S>
constexpr S Max(const S& a, const S& b) {
    return std::max(a, b);
}
template <typename S, std::enable_if_t<std::is_scalar_v<S>>* = nullptr>
constexpr S MinLimit() {
    return std::numeric_limits<S>::lowest();
};

template <typename S>
using RangeMaxQuery = atcoder::segtree<S, Max<S>, MinLimit<S>>;
...
int main() {
    ...
    RangeMaxQuery<long long> seg(200001);
    seg.set(0, 0);
    for (int i = 0; i < N; i++) {
        seg.set(A[i], std::max(seg.get(A[i]), seg.prod(0, A[i] + 1) + B[i]));
    }
