...
[[nodiscard]] static constexpr bool is_prime32(std::uint32_t x) noexcept {
...
class modint_base {
   public:
    ...
    /// @param n 指数
    /// @return 自身の @c n 乗
    [[nodiscard]] constexpr Modint pow(unsigned long long n) const noexcept {
        Modint x = 1;
    ...
    [[nodiscard]] friend constexpr Modint operator+(const Modint& x, const Modint& y) noexcept {
    ...
    [[nodiscard]] friend constexpr bool operator==(const Modint& x, const Modint& y) noexcept {
...
class static_modint : public modint_base<static_modint<Mod>> {
   ...
   public:
    ...
    /// @brief 法を取得します。
    /// @return 法
    [[nodiscard]] static constexpr value_type mod() noexcept {
        return Mod;
    }
    ...
    /// @remark 時間計算量： @f$O(\log value)@f$
    /// @return 自身の逆数
    [[nodiscard]] constexpr static_modint inv() const noexcept {
...
class dynamic_modint : public modint_base<dynamic_modint<ID>> {
   ...
   public:
    ...
    /// @brief 法を取得します。
    /// @return 法
    [[nodiscard]] static value_type mod() noexcept {
        return modulus;
    }
...
void solve() {
    ...
    MergeSortTree<mint, int, std::multiplies<mint>, mintE, mintInv> mstree(value, key);
    while (Q--) {
        int L, D, R, U;
        std::cin >> L >> D >> R >> U;
